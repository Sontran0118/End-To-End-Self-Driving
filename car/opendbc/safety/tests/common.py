        ...
        if attr.startswith("Test") and attr != current_test:
          ...
          if tx is not None and not attr.endswith('Base'):
            ...
            if attr.startswith('TestHyundaiCanfd') and current_test.startswith('TestHyundaiCanfd'):
              continue
            # Mazda's alpha-long mode is a superset of the stock tx table, so the
            # stock class's messages are legitimately transmittable under it.
            # Only the addresses alpha long ADDS should be checked across modes.
